...
def get_hadK_block(n, block_size=16, transpose=False):
    assert n % block_size == 0, "n must be divisible by block size!"
    K = n // block_size
    H = scipy.linalg.hadamard(block_size) / (block_size ** 0.5)
    hadK = torch.tensor(H, dtype=torch.float32)  # or whatever dtype needed
    if transpose:
        hadK = hadK.T
    return hadK, K
