...
class Qwen2MLP(nn.Module):
    ...
    def __init__(
        ...
        elif fake_quant_config['cfg']=='int4_g128':
            cfg = {'num_bits': 4, 'block_sizes': {-1: 128}, 'enable': True}
        else:
            raise NotImplementedError
        self.quantizer = TensorQuantizer(cfg, if_calib=True)

    def forward(self, x):
        x = self.quantizer._fake_quantize(x)
...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.quantizer._fake_quantize(hidden_states)
        qkv, _ = self.qkv_proj(hidden_states)
        q, k, v = qkv.split([self.q_size, self.kv_size, self.kv_size], dim=-1)
        q, k = self.rotary_emb(positions, q, k)
        if self.kv_quant:
            k = self.quantizer._fake_quantize(k)
            v = self.quantizer._fake_quantize(v)
        attn_output = self.attn(q, k, v)
        attn_output = self.quantizer._fake_quantize(attn_output)
