...
class TensorQuantizer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self._rotate:
            # Check if block rotation is enabled
            block_size = getattr(self, "_block_rotation_size", None)
            
            if block_size is not None and block_size > 0:
                # Use deterministic block-wise Hadamard transform
                n = inputs.shape[-1]
                if n % block_size == 0:
                    # Reshape to (..., n // block_size, block_size)
                    original_shape = inputs.shape
                    inputs_reshaped = inputs.reshape(-1, n // block_size, block_size)
                    
                    # Apply normalized Hadamard transform to each block
                    try:
                        import fast_hadamard_transform
                        scale = 1.0 / torch.sqrt(torch.tensor(block_size, dtype=torch.float32, device=inputs.device))
                        inputs_reshaped = fast_hadamard_transform.hadamard_transform(
                            inputs_reshaped.contiguous(), scale
                        )
                    except ImportError:
                        # Fallback: apply normalized Hadamard to each block manually
                        for i in range(inputs_reshaped.shape[1]):
                            inputs_reshaped[:, i, :] = normalized_hadamard_transform(inputs_reshaped[:, i, :])
                    
                    inputs = inputs_reshaped.reshape(original_shape)
                else:
                    # Fallback to full dimension if not divisible by block_size
                    warnings.warn(
                        f"Input dimension {n} is not divisible by block_size {block_size}. "
                        "Falling back to full dimension Hadamard transform."
                    )
                    inputs = normalized_hadamard_transform(inputs)
            else:
                # Use full dimension Hadamard transform (original behavior)
                inputs = normalized_hadamard_transform(inputs)
    ...
    def extra_repr(self):
        ...
        if self._rotate:
            block_size = getattr(self, "_block_rotation_size", None)
            if block_size is not None and block_size > 0:
                s += f" block_rotated(block_size={block_size})"
            else:
                s += " rotated"
        s += (
